...
Тот же конвейер можно собрать из скилов Claude Code, без отдельного
инструмента: в [Superpowers](https://github.com/obra/superpowers) —
`brainstorming` → `writing-plans` → реализация сабагентами с TDD-циклом; в
[паке Мэтта Покока](https://github.com/mattpocock/skills) — `/to-spec` →
`/to-tickets` → `/implement`. Подробный разбор обоих паков — в главе
...
- [GitHub Spec Kit](spec-kit.md), [OpenSpec](openspec.md), [Kiro](kiro.md) и
  [Tessl](tessl.md) — четыре фреймворка, разобранные в статьях этого раздела;
  манифест SDD как методологии — в
