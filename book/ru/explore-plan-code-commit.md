...
1. Включите режим планирования: до утверждения плана агент не сможет менять
   файлы, и запрещать код в промптах не нужно.
2. Отдайте задачу и попросите план. Переписывать постановку в промпт не
   обязательно — достаточно скинуть тикет из трекера: описание и релевантный
   код агент прочитает сам.
3. Прочитайте план как ревью: задавайте вопросы, вычёркивайте лишнее, требуйте
   альтернатив, проговаривайте ограничения, которых не видно в коде.
   Итерируйте до согласия — это самая дешёвая фаза для споров.
4. Утвердите план штатным подтверждением инструмента и укажите, чем агент
   может проверить себя: тесты, сборка, линтер.
...
В беклоге лежит тикет: «в CSV-экспорте отчётов время сдвинуто на час у части
пользователей». Разработчик включает режим планирования — до утверждения плана
агент не сможет менять файлы — и отдаёт тикет как есть:

> Разберись с REP-1432 и составь план исправления.

**Исследование:** агент читает тикет и код экспорта отчётов и находит, где
время конвертируется при записи в CSV.

**План:** агент предлагает два варианта — конвертировать время при записи или
при чтении. Разработчик отвечает:

> Вариант с конвертацией на чтении ломает уже выгруженные файлы — формат
> читают внешние интеграции. Берём первый вариант, добавь в план тест на
> границу перехода на летнее время.

**Код:** разработчик принимает исправленный план штатным подтверждением
инструмента — агент выходит из режима планирования, реализует план и прогоняет
тесты экспортёра.
...
- [Спеко-ориентированная разработка](spec-driven-development.md) — тот же
  принцип «сначала договориться, потом кодить», развёрнутый в артефакты,
  которые переживают сессию: спецификация → план → задачи → реализация.
